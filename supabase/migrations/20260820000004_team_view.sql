...
begin
  ...
  if v_team.status = 'playing' and v_status = 'live' and v_team.current_position < v_total then
    ...
    -- Teams already through the checkpoint before this race (current_position 0 never
    -- counts as "through" anything, so the opening race always starts at taken = 0).
    if v_level <= 1 then
      v_taken := 0;
    else
      select count(*)::int into v_taken from teams where current_position >= v_level - 1;
    end if;
